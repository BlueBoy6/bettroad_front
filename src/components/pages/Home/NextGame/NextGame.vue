	...
	<div>
		...
		>
			...
			<NextGameAlreadyBetted
				...
				:nextGameBetted="nextGameBetsList"
			/>
			<!-- SI AUCUN PARIS N'A ETE SOUMIS POUR CE GAMEDAY -->
			<v-row justify="end" v-else>
				<v-col v-if="nextGameBetsList.length > 0">
					<v-card
						:key="bet.id"
				...
				<v-col v-else>
					<v-sheet
						justify="center"
						:class="[colorSuccess, whiteText, spaceInside]"
					>
						<p class="subtitle-1">
							Il n'existe toujours aucun paris pour ce jour de match..
						</p>
						<p class="headline">
							Ca vient, ça vient ! 🏒
						</p>
